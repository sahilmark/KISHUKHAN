...
def log_cls(log):
    ...
    if 'âŒ' in lo  or 'error' in lo or 'fail' in lo or 'fatal' in lo: return 'le'
...
def send_loop(config, s, pid='AUTO-1'):
    ...
    try:
        ...
        if not inp:
            lg(f'{pid}: âŒ Message input not found!', s); s.running=False; return 0
        ...
        lg(f'{pid}: âŒ Fatal: {str(e)}', s); s.running=False; return 0
    finally:
        if driver:
...
def start_auto(config):
    ...
    if st.session_state.cookie_mode=='multiple' and st.session_state.multi_cookies:
        ...
        t = threading.Thread(target=run_multi, args=(cfgs,s), da
